...
OperationQueue::add_operation(
    ...
    SPDLOG_LOGGER_INFO(logger, "Trying to add operation: type={}, idx = {}", static_cast<int>(operation.type), idx);
...
OperationQueue::get_all_operations(void* buffer) {
    ...
    ssize_t count = _operation_count.load(std::memory_order_relaxed);
    if (count > 0) {
        memcpy(buffer, _operation_queue, count * sizeof(Operation_t));
    }
    _key_to_index.clear();
    _operation_count = 0;
    SPDLOG_LOGGER_DEBUG(logger, "Getting all operations, count = {}", count);
    return count;
}